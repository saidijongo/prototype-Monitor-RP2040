...
void motorControl(String motorCommand) {
  ...
  Serial.print("ST,0,MOTOR,0, current pos " + String(_step_current_position) + " ,mapped angle " + String(mappedAngle) + ",ED\r\n");
  ...
  while (_step_current_position != _step_target_position) {
    ...
    motorStep(direction == 1, 1, _step_current_position); // Adjusted to take only one step at a time
...
void commandProcess(String commandStr) {
  ...
  if (cmd.equals("MOTOR")) {
    ...
    Serial.print("ST,0," + cmd + ",ERR_INVALID");
  }
}
