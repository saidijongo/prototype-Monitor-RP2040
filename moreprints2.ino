...
void homePosition() {
  ...
  while (digitalRead(TOP_SENSOR_PIN) == HIGH && digitalRead(BOTTOM_SENSOR_PIN) == HIGH && rotationCount <= 200) {
    motorStep(true, 1); //CW
    rotationCount++;
  ...
  Serial.print("step current: " + String(_step_current_position) + "," + ",ED\r\n");
...
void motorControl(String motorCommand) {
  ...
  while (_step_current_position != _step_target_position) {
    ...
    motorStep(direction == 1, 1); //only one step at a time
  }
